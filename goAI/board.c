...
{
	int num_stone=0,i,j,legal_moves[MAX_BOARD*MAX_BOARD];
	for (i=0;i<=MAX_BOARD*MAX_BOARD-1;++i)
	{
		if (bs->board[i]!=EMPTY) num_stone++;
	}

	//DpPrintf("num of legal moves: %d \n",bs->legal_moves_num);
	if (num_stone<=30 ) return 0;
...
{
    int ai, aj, bi, bj, k,stage,good_moves[MAX_BOARD*MAX_BOARD],best_moves[MAX_BOARD*MAX_BOARD],num_best_moves=0,num_good_moves;
    int num_moves = 0;
    ...
    for (ai = 0; ai < board_size; ai++)
        for (aj = 0; aj < board_size; aj++) {
            ...
            if (legal_move(bs, ai, aj, color)
                 ...
                 */
                if (!suicide(bs, ai, aj, OTHER_COLOR(color)))
                    if (aj>=2 && aj<=MAX_BOARD-3 &&aj>=2 &&ai<=MAX_BOARD-3) bs->legal_moves[num_moves++] = POS(ai, aj);
                else {
                    /* ...however, if the move captures at least one stone,
                     * consider it anyway.
                     */
                    for (k = 0; k < 4; k++) {
                        ...
                        if (ON_BOARD(bi, bj)
                            && bs->board[POS(bi, bj)] == OTHER_COLOR(color)) {
								...
								if (ai!=0 && aj!=0 && ai!=MAX_BOARD-1 && aj!=MAX_BOARD-1) bs->legal_moves[num_moves++] = POS(ai, aj);
                            break;
                        }
                    }
                }
    ...
    for (ai = 0; ai < board_size; ai++)
        for (aj = 0; aj < board_size; aj++) {
            ...
            if (legal_move(bs, ai, aj, color)
                 ...
                 */
                if (!suicide(bs, ai, aj, OTHER_COLOR(color)))
                    //good_moves[num_good_moves++]=POS(ai,aj);
...
int same_string(board_status *bs, int pos1, int pos2)
{
...
int max_liberty_string(board_status *bs,intersection color)
{
	int newboard[MAX_BOARD * MAX_BOARD];
	int max_liberty_stones=0;
	int current_liberty_stones;
	int maxi,maxj,cnt,ai,aj;
	for (cnt=0;cnt<=board_size * board_size-1;++cnt)
	{
		newboard[cnt]=1;
	}
	for (ai = 0; ai <= board_size-1; ai++)
		for (aj = 0; aj <= board_size-1; aj++) {
			int pos=POS(ai,aj);
			if (bs->board[POS(ai, aj)] == color && newboard[pos]==1 )
			{
				current_liberty_stones=liberty_stones_rate(bs,ai,aj);
				if (current_liberty_stones>max_liberty_stones) {max_liberty_stones=current_liberty_stones;maxi=ai;maxj=aj;}
			}
		}
	return POS(maxi,maxj);
}

int liberty_string(board_status *bs,int ai,int aj) //output the liberty of a string
{
	int i,j,k,ii,jj,pos;
	int num_liberty=0,liberty[MAX_BOARD * MAX_BOARD];
	for (i = 0; i <= MAX_BOARD * MAX_BOARD-1; i++) liberty[i]=0;
	pos=POS(ai,aj);
	do {
    i = I(pos);
    j = J(pos);
	for (k = 0; k < 4; k++) {
      ii = i + deltai[k];
      jj = j + deltaj[k];
	  if (ON_BOARD(ii, jj) && bs->board[POS(ii, jj)] == EMPTY && liberty[POS(ii,jj)]==0) {num_liberty++;liberty[POS(ii,jj)]=1;}
    }
    pos = bs->next_stone[pos];
  } while (pos != POS(ai, aj));
...
int liberty_stones_rate(board_status *bs,int ai,int aj)    //ouput 10*num_stone-num_liberty of a string
{
	...
	num_liberty=liberty_string(bs,ai,aj);
	return 10*num_stone-num_liberty;
}
